...
class Calendar {  
    ...
    */
			...
			foreach($event_list as $list)
			{
				$content.= "<div class='event_detail'>
						<div class='event_date'>"
							.strtoupper(date("M d", strtotime($list['date_start'])))
					  	."</div>
	...
	{
		$event_count = dbc_query_one("SELECT event_id FROM events
										WHERE DATE(date_start) = '{$date}'
										OR DATE(date_end) = '{$date}'
										OR (DATE(date_start) < '{$date}' AND DATE(date_end) > '{$date}')");
		//$event_count = dbc_query_one("SELECT event_id FROM events WHERE DATE(date_start) = {$date} OR ($date between DATE(date_start) AND DATE(date_end) <={$date}");
		return $event_count;
	}
	
	public function fetchAllEvent($year=null, $month=null)
	{
		$event_count = dbc_query_all("SELECT * FROM events WHERE YEAR(date_start) = '{$year}' AND  MONTH(date_start) = '{$month}'");
		return $event_count;
	}
}
?>